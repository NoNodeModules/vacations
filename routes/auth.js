...
router.put('/login', async (req, res) => {
    ...
    try {
        const select_query = `SELECT id,role,username,password,signedIn FROM users WHERE username="${username}"`
        console.log("hey")
...
console.log("hey")
        ...
        const signedInUser_q = `SELECT * FROM users WHERE username = "${user[0].username}"`
...
router.put('/logout', async (req, res) => {
    ...
    try {
        const q = `SELECT* FROM users WHERE id="${storedToken.id}"`
